...
const VehicleForm = ({ params, searchParams }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const key in formData) {
      ...
      } else if (key === 'maintenance_points' || key === 'mechanical_results' || key === 'other_options' || key === 'accessories_options') {
      ...
      <div className={`  flex-1 flex flex-col overflow-hidden transition-all duration-300`}>
        ...
        <div className="flex overflow-auto p-8">
          <div className="container bg-black mx-auto p-4">
            <h1 className="text-2xl font-bold mb-4 text-amber-600">Add New Vehicle</h1>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-amber-600/20 p-6 bg-gray-900 rounded-lg text-center">
                  ...
                  <p className="text-sm text-red-500 mb-2">1. Max. 60 Images Allow on create page and only jpg, jpeg, png, & bmp format Allow and Max. File Size 5MB Allow.</p>
                  ...
                  {Object.keys(formData.maintenance_points).map((key) => (
                    <div key={key}>
                      <input
                        ...
                        className="mr-2"
                      />
                      <label className="text-sm text-white">{key.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</label>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                {Object.entries(formData.accessories_options).map(([category, fields]) => (
                  <div key={category} className="mb-4">
                    <h3 className="text-md font-medium text-amber-600 capitalize">{category.replace(/_/g, ' ')}</h3>
                    <div className="grid grid-cols-2 gap-2">
                      {Object.keys(fields).map((field) => (
                        <div key={field}>
                          <input
                            ...
                            className="mr-2"
                          />
                          <label className="text-sm text-white">{field.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</label>
                        </div>
                      ))}
                    </div>
                  </div>
